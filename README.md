...
| Spring Boot 后端 | `scripts\dev\backend-run-jar.cmd` | `scripts\dev\backend-stop.cmd` | 打开 `http://127.0.0.1:8080/actuator/health`，看到 `UP` 即正常 |
...
- `backend-build.cmd` 会编译 Spring Boot 后端并生成可运行 jar。

## 启动顺序

推荐先使用一键本地启动脚本，它会按“数据库 -> 后端 -> 前端”的顺序启动服务，并把后端、前端放到后台运行：
...
如果已经执行过 `scripts\dev\backend-build.cmd`，推荐使用 jar 启动：

```powershell
scripts\dev\backend-run-jar.cmd
```

如果正在开发后端代码，也可以使用 Maven 开发模式：
...
- 工具栏“实体”菜单中的“立方体”“球体”“锥体”可以直接按参数创建三维实体；立方体/长方体输入长宽高，球体输入半径，锥体输入底面半径和高度。
...
- 常用建模快捷键：`S` 选择，`L` 线段，`R` 矩形，`C` 圆，`A` 圆弧，`D` 尺寸，`H` 水平，`V` 垂直，`T` 相切，`Q` 切换构造；`Shift+E` 拉伸，`Shift+X` 切除。
...
- 在对象树中选中拉伸或切除特征后，可以在右侧属性面板修改名称、抑制状态和深度。
- 在对象树中选中布尔特征后，可以在右侧属性面板切换 Union / Difference。
- 在对象树或三维视口中选中立方体、球体、锥体后，可以拖拽移动实体；右侧属性面板可以精确修改实体坐标、尺寸、半径和高度。
- 在三维视口或左侧对象树中按住 `Ctrl` / `Shift` 点击三维特征，可以同时选中多个球体、立方体、锥体或布尔结果。
- 多选两个三维实体后，工具栏“实体 -> Union / Difference / Cut”会按选择顺序执行布尔操作：第一个选中对象作为目标，第二个选中对象作为工具体。
- “实体 -> Cut”有两种模式：选中两个三维实体时执行实体差集切除；选中矩形/圆草图且已有三维实体时，用该草图轮廓切除最近生成的三维结果。
